...
class ExportService {
  static Future<void> exportToPDF(BuildContext context) async {
    try {
      ...
      if (kIsWeb) {
        _downloadFileWeb(bytes, 'cv_${DateTime.now().millisecondsSinceEpoch}.pdf', 'application/pdf');
      } else {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(kIsWeb ? 'PDF downloaded successfully!' : 'PDF exported successfully!'),
  ...
  static Future<void> exportToAdvancedPDF(BuildContext context) async {
    try {
      ...
      if (kIsWeb) {
        _downloadFileWeb(bytes, 'cv_advanced_${DateTime.now().millisecondsSinceEpoch}.pdf', 'application/pdf');
      } else {
        final dir = await getTemporaryDirectory();
        final file = File(
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(kIsWeb ? 'Advanced PDF downloaded successfully!' : 'Advanced PDF exported successfully!'),
            action: SnackBarAction(label: 'OK', onPressed: () {}),
          ),
        );
      }
  ...
  static Future<void> exportToDOCX(BuildContext context) async {
    try {
      ...
      if (kIsWeb) {
        _downloadFileWeb(bytes, 'cv_${DateTime.now().millisecondsSinceEpoch}.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      } else {
        final dir = await getTemporaryDirectory();
        final file = File(
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(kIsWeb ? 'DOCX downloaded successfully!' : 'DOCX exported successfully!'),
            action: SnackBarAction(label: 'OK', onPressed: () {}),
          ),
        );
      }
  ...
  static Future<void> exportToHTML(BuildContext context) async {
    try {
      ...
      if (kIsWeb) {
        _downloadFileWeb(htmlContent.codeUnits, 'cv_${DateTime.now().millisecondsSinceEpoch}.html', 'text/html');
      } else {
        final dir = await getTemporaryDirectory();
        final file = File(
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(kIsWeb ? 'HTML downloaded successfully!' : 'HTML exported successfully!'),
            action: SnackBarAction(label: 'OK', onPressed: () {}),
          ),
        );
      }
  ...
  static void _downloadFileWeb(List<int> bytes, String fileName, String mimeType) {
